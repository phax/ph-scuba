...
/**
 * SPI interface for content validation based on file extension. Implementations are discovered via
 * {@link java.util.ServiceLoader} and registered in
...
{
  /**
   * @return The set of file extensions this validator can handle. Each extension must start with a
  ...
  /**
   * Initialize this validator with the repository storage. Called once after SPI discovery.
   *
   * @param aRepo
   *        The repository storage to use for resolution checks. May not be <code>null</code>.
   */
   ...
   * @param aIS
   *        The input stream to read the content from. Will be closed by the caller. Never
   *        <code>null</code>.
   * @param aErrorList
   *        The error list to collect all validation errors into. Never <code>null</code>.
   * @return {@code true} if the content is valid for this file type, {@code false} otherwise.
   * @throws IOException
   *         On IO error
   */
